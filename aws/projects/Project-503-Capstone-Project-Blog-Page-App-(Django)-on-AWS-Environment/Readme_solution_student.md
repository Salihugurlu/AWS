...
### Step 1: Create dedicated VPC and whole components
    ...
    - ATTACH the internet gateway `aws_capstone-IGW` to the newly created VPC `aws_capstone-VPC`. Go to IGW and select newly created IGW and click action ---> Attach to VPC ---> Select `aws_capstone-VPC` VPC 
...
Protocol            : HTTP
Domain Name         : Write cloudfront domain name (d2n2hzyj6j3ier.cloudfront.net)
Port                : 80
Path                : leave it blank
Other stuff         : Keep them as are
```
...
Create IAM role         : aws_capstone_lambda_Role

Advance Setting:
Network                 : 
    - VPC               : aws-capstone-VPC
    - Subnets           : Select all subnets
    - Security Group    : Select default security Group (inbound allows all traffic)
```
...
Go to the S3 console and select the S3 bucket named `awscapstonec3<name>blog` (muratblog).

- Go to the properties menu ---> Go to the Event notifications part

> We can pass 559-570 if we add trigger in Lambda Function Test Page <
